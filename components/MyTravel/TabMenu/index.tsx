...
function TabMenu({ selectTab, onTabChange }: TabMenuProps) {
  return (
    <div className="absolute top-120 z-10 flex h-72 items-center justify-center rounded-36 bg-white px-12 py-10 shadow-md tablet:h-64  mobile:top-130 mobile:h-56 ">
      <div className="flex items-center justify-center gap-8">
        ...
        <button
          className={`line-height-130 letter-spacing--0.6 flex h-60 w-150 items-center justify-center text-center text-18 font-bold tablet:h-52 tablet:w-130 mobile:h-44 mobile:w-94 mobile:text-14 ${selectTab === "최근 본 글" ? "rounded-36 bg-[#E0E7FF] text-[#6366F1]" : ""}`}
          onClick={() => onTabChange("최근 본 글")}
        >
          최근 본 글
        </button>
        <button
